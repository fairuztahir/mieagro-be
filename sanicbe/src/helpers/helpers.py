...
def convertISO_TZ(value):
    ...
    iso = value.astimezone(ZoneInfo(tz.fulltext))
    # return iso.isoformat()
    return moment.date(iso).format(fmt.fulltext)
...
async def is_valid_date(date):
    if date:
        try:
            ...
            expected = moment.date(parsed_value).format(fmt.fulltext)
