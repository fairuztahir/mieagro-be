...
from helpers.func import valueOf, exceptionRaise, is_valid_date, resJson, resType
...
class DownloadCSVView(HTTPMethodView):
    ...
    async def get(request):
        try:
            ...
            # TODO: Call a function to get rows data, will remove Odoo feature soon - now commented and removed
            ...
            file_date = None
            if date:
                file_date = arrow.get(date).format(fmt.fulltext)
            else:
                utc = arrow.utcnow()
                file_date = utc.to(tzone.fulltext).format(fmt.fulltext)
